...
public class ShapeInterface extends JComponent implements ActionListener,ChangeListener {
	...
	private int dimQty[] = {300, 300, 300, 10, 6};
	...
	public ShapeInterface(){
		...
		fr.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);  //Close window on exit
	 	...
	 	save = new JButton("Save Shape to Image");
		...
		topPanel1.add(dimLabels[4]);
		topPanel1.add(dimFields[4]);		
		removeAllFields();
		...
		//For testing
		shapes.setSelectedIndex(shapes.getItemCount() - 1);
	...
	public void paintComponent(Graphics g){
		...
		if(shapes.getSelectedItem() == "Square"   ){
		  ...
		  g.fillRect(fr.getWidth()/2 - dimQty[0]/2, fr.getHeight()/2 - dimQty[0]/2 - topOuterPanel.getHeight(), 
			...
			dimFields[4].setVisible(true);
			dimLabels[4].setVisible(true);
			
			int sides = dimQty[4];
			int length = dimQty[0];
			
			int xPts[] = new int[sides];
			int yPts[] = new int[sides];
			
			double theta = 2 * Math.PI / sides;
			for (int i = 0; i < sides; ++i) {
			    double x = Math.cos(theta * i)*length ;
			    double y = Math.sin(theta * i)*length ;
			    xPts[i] = (int)x + fr.getWidth()/2 ;
			    yPts[i] = (int)y + fr.getHeight()/2 - length/2;    
			}
			
			g.fillPolygon(xPts, yPts, sides);
		}
	}
	...
	public void saveImage(){
        ...
        try {
            ImageIO.write(img, "png", new File(shapes.getSelectedItem() + "-color.png"));
